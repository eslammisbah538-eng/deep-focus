...
const SOUND_DEFS = [
    { id: 'rain', name: 'مطر', desc: 'مطر خفيف على النافذة', color: '#3b82f6', vol: 70, icon: 'cloud-rain' },
    { id: 'ocean', name: 'أمواج البحر', desc: 'أمواج بحرية مهدئة', color: '#06b6d4', vol: 60, icon: 'waves' },
    { id: 'nature', name: 'طبيعة', desc: 'أصوات الغابة والطيور', color: '#22c55e', vol: 65, icon: 'leaf' },
    { id: 'cafe', name: 'كافيه', desc: 'أجواء المقهى', color: '#f97316', vol: 55, icon: 'coffee' },
    { id: 'fire', name: 'مدفأة', desc: 'نار متقدة دافئة', color: '#ef4444', vol: 60, icon: 'flame' },
    { id: 'lofi', name: 'لو-فاي', desc: 'موسيقى دراسة هادئة', color: '#8b5cf6', vol: 50, icon: 'music' },
    { id: 'night', name: 'أصوات الليل', desc: 'صراصير وهدوء الليل', color: '#6366f1', vol: 55, icon: 'moon' },
];
...
let selectedAvatarColor = AVATAR_COLORS[5];
let G = {
...
function makeDefault(name, color) { return { name, avatarColor: color, subjects: [], sessions: [], flashDecks: [], streak: { count: 0, lastDate: '' }, totalCardReviews: 0 }; }
...
function buildAvatarPicker() {
    ...
    if (!el) return;
    el.innerHTML = AVATAR_COLORS.map(c => `<div class="av-opt${c === selectedAvatarColor ? ' selected' : ''}" style="background:${c}" onclick="selectAvatar('${c}',this)"></div>`).join('');
}
function selectAvatar(color, el) {
...
function handleEntry() {
    ...
    if (!name) {
        if (er) { er.textContent = 'من فضلك أدخل اسمك'; er.style.display = 'block'; }
        return;
    }
    ...
    let saved = (existing && sameAccount) ? existing : makeDefault(name, selectedAvatarColor);
    saved.name = name;
    saved.avatarColor = selectedAvatarColor;
    try {
...
function renderAIHelp() {
    ...
    let greet = '';
    if (hour < 5) greet = `<i data-lucide="moon"></i> ${name}، ليلة عمل جاد`;
    else if (hour < 12) greet = `<i data-lucide="sun"></i> صباح التفوق يا ${name}`;
    else if (hour < 17) greet = `<i data-lucide="coffee"></i> مساء الإنجاز يا ${name}`;
    else greet = `<i data-lucide="home"></i> مساء الخير يا ${name}`;
    ...
    // ── اقتراحات ديناميكية مخصصة بناءً على البيانات الفعلية
    const sugs = [];

    // اقتراح 1: أكثر إلحاحاً حسب الموقف
    if (urgentSub && daysUntil(urgentSub.examDate) !== null && daysUntil(urgentSub.examDate) <= 7) {
        // مادة قريبة — اسأل عن الهدف اليومي المحسوب مش الساعات الكلية
        sugs.push(`كام دقيقة المفروض أذاكر ${urgentSub.name} النهارده؟`);
    } else if (todayMin === 0) {
        sugs.push(`ابني لي جدول اليوم من دلوقتي`);
    } else {
        sugs.push(`إيه أولويتي دلوقتي؟`);
    }

    // اقتراح 2: تحليل الأداء الفعلي
    if (weekly.totalHours > 0) {
        sugs.push(`قيّم أداء الأسبوع ده بصراحة`);
    } else {
        sugs.push(`ازاي أبدأ وأنا مش عارف من فين؟`);
    }

    // اقتراح 3: الاختبار — فقط في المواد النشطة اللي لسه امتحانها جاي
    const activeWithExam = activeSubs.filter(s => s.examDate && daysUntil(s.examDate) !== null && daysUntil(s.examDate) > 0);
    if (activeWithExam.length > 0) {
        sugs.push(`اختبرني في ${activeWithExam[0].name}`);
    } else if (activeSubs.length > 0) {
        sugs.push(`اسألني أسئلة في ${activeSubs[0].name}`);
    } else {
        sugs.push(`اختبرني في أي موضوع`);
    }

    // اقتراح 4: تحفيز مخصص أو نقاط ضعف
    if (totalHours < 5) {
        sugs.push(`حسسني إن المذاكرة مش صعبة`);
    } else if (weekly.activeDays <= 3) {
        sugs.push(`ليه بقيت مش منتظم الأسبوع ده؟`);
    } else {
        sugs.push(`فين نقاط ضعفي الحقيقية؟`);
    }

    // اقتراح 5: حسب الوقت — صيغة تخليه يستخدم الهدف اليومي مش يحسب بنفسه
    if (hour >= 22 || hour < 5) {
        sugs.push(`هل المذاكرة وقت متأخر مفيدة؟`);
    } else if (urgentSub) {
        sugs.push(`وزّع لي مذاكرة كل مادة على الأيام الجاية`);
    } else {
        sugs.push(`نصيحة واحدة تخليني أذاكر أكتر`);
    }
...
async function sendAIMessage(text) {
    ...
    msgs.innerHTML += `<div class="ai-msg user"><div class="ai-msg-av" style="background:${G.data.avatarColor || '#3b82f6'}">${(G.data.name || 'U')[0].toUpperCase()}</div><div class="ai-msg-bubble" style="direction:rtl;unicode-bidi:plaintext;text-align:right">${text.replace(/</g, '&lt;')}</div></div>`;
